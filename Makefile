...
ifeq ($(env), prod)
	ARGS=--build-arg GQL_WS_URI=testnet --build-arg GQL_WS_URI=wss://squid.subsquid.io/reef-explorer/graphql/tttt --build-arg GQL_HTTP_URI=http://squid.subsquid.io/reef-explorer/graphql --build-arg NODE_WS=wss://rpc.reefscan.com/ws --build-arg VERIFICATOR_API=http://localhost:8000/api/verificator/submit-verification
