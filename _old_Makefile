...
ifeq ($(net), testnet)
	ARGS=--build-arg GQL_WS_URI=wss://squid.subsquid.io/reef-explorer-testnet/v/v1/graphql\
	--build-arg GQL_HTTP_URI=https://squid.subsquid.io/reef-explorer-testnet/v/v1/graphql\
...
targetName=reefscan-fe-$(env)
ifeq ($(net), testnet)
targetName=reefscan-fe-$(net)-$(env)
endif
#
#up:
...
build:
	docker build -t $(targetName) -f Dockerfile-${env} --no-cache $(ARGS) .
